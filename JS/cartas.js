...
async function cargarDatos(tipo) {
    try {
        ...
        /* Sin where()/orderBy() en el servidor: un documento sin el
           campo "orden" quedaría invisible si Firestore ordenara.
        ...
        const [catSnap, platoSnap] = await Promise.all([
            ...
            getDocs(collection(db, COLECCION[tipo]))
        ]);

        const categorias = catSnap.docs.map(d => d.data())
            .filter(c => c.menu === clave)
            .sort((a, b) => (a.orden ?? Infinity) - (b.orden ?? Infinity))
            .map(c => c.nombre);
        const platos = platoSnap.docs.map(d => ({ id: d.id, ...d.data() }))
